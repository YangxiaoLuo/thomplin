...
// a grid diagram not reponding
// const int ArcIndex = 22;
// char Xs[ArcIndex]={22,7,5,19,20,9,4,17,21,11,10,15,16,8,3,13,14,6,2,12,1,18};
// char Os[ArcIndex]={1,21,22,9,7,17,19,10,8,16,18,11,5,14,15,6,4,12,13,3,20,2};

// const int ArcIndex = 26;
// char Xs[ArcIndex]={26,7,5,24,25,13,11,18,20,9,4,14,3,15,21,10,8,17,22,12,6,16,2,19,1,23};
// char Os[ArcIndex]={1,25,26,11,9,22,23,13,7,14,15,5,18,6,4,17,20,12,10,16,19,8,21,3,24,2};

/* Figure 2 */
/* m(10_132) L_1: LL not null-homologous, UR null-homologous */
const int ArcIndex = 10;
...
void change_Xs(State newXs)
{
  for (int i; i<ArcIndex; i++)
  {
    Xs[i] = newXs[i];
  }
}

void change_Os(State newOs)
{
  for (int i; i<ArcIndex; i++)
  {
    Os[i] = newOs[i];
...
int is_URnull()
{
  ...
  return (NullHomologousD0Q(UR));
}

int is_D1LLnull()
{
  return (NullHomologousD1Q(Xs));
}
...
void SpecialHomology(int init, int final)
{
  ...
  while((EdgeList!=NULL)&&(Temp!=NULL)) {
    ...
    if (j==100) {  
      j =0; 
      if (Temp!=NULL) printf("Iteration number %d; Contracting edge starting at (%d,%d)\n",i, Temp->start, Temp->end);
    };
    i++;
    j++;
    if (Temp!=NULL)  { 
